...
// pivotI4 divides [lo..hi] range into 2n+1 equal intervals, sorts mid-points of them
...
func pivotI4(ar []int32, lo, hi, n int) (int, int32, int) {
	m := mid(lo, hi)
...
func partition1I4(ar []int32, l int, pv int32, h int) int {
	for {
...
func partition2I4(ar []int32, l, a int, pv int32, b, h int) (int, int) {
	for {
...
// returns m with ar[:m] <= pivot, ar[m:] >= pivot
func cdualparI4(par chan int, ar []int32, lo, hi int) int {

	lo, pv, hi := pivotI4(ar, lo, hi, 4) // median-of-9

	m := mid(lo, hi)
	a, b := mid(lo, m), mid(m, hi)

	go func(l, h int) {
		par <- partition1I4(ar, l, pv, h) // mid half range
	}(a, b)

	a, b = partition2I4(ar, lo, a-1, pv, b+1, hi) // left/right quarter ranges
	m = <-par
...
func shortI4(ar []int32, lo, no int) {
start:
	n := lo + no
	l, pv, h := pivotI4(ar, lo, n, 2) // median-of-5
	l = partition1I4(ar, l, pv, h)
	n -= l
	no -= n + 1
	...
	if n >= Mli {
		shortI4(ar, l, n) // recurse on the shorter range
		goto start
	}
	insertionI4(ar[l:], n) // at least one insertion range

	if no >= Mli {
		goto start
	}
	insertionI4(ar[lo:], no) // two insertion ranges
	return
}
...
func SortI4(ar []int32) {
	...
	start:
		n := lo + no
		l, pv, h := pivotI4(ar, lo, n, 3) // median-of-7
		l = partition1I4(ar, l, pv, h)
		n -= l
		no -= n + 1
		...
		if n < Mlr { // at least one not-long range?
			...
			shortI4(ar, lo, no) // we know no >= Mli
			return
		}
