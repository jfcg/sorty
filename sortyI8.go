...
func insertionI8(ar []int64, hi int) {
...
// pivotI8 divides [lo..hi] range into 2n+1 equal intervals, sorts mid-points of them
...
func pivotI8(ar []int64, lo, hi, n int) (int, int64, int) {
	m := mid(lo, hi)
...
func partition1I8(ar []int64, l int, pv int64, h int) int {
	for {
...
func partition2I8(ar []int64, l, a int, pv int64, b, h int) (int, int) {
	for {
...
// returns m with ar[:m] <= pivot, ar[m:] >= pivot
func cdualparI8(par chan int, ar []int64, lo, hi int) int {

	lo, pv, hi := pivotI8(ar, lo, hi, 4) // median-of-9

	m := mid(lo, hi)
	a, b := mid(lo, m), mid(m, hi)

	go func(l, h int) {
		par <- partition1I8(ar, l, pv, h) // mid half range
	}(a, b)

	a, b = partition2I8(ar, lo, a-1, pv, b+1, hi) // left/right quarter ranges
	m = <-par
...
func shortI8(ar []int64, lo, no int) {
start:
	n := lo + no
	l, pv, h := pivotI8(ar, lo, n, 2) // median-of-5
	l = partition1I8(ar, l, pv, h)
	n -= l
	no -= n + 1
	...
	if n >= Mli {
		shortI8(ar, l, n) // recurse on the shorter range
		goto start
	}
	insertionI8(ar[l:], n) // at least one insertion range

	if no >= Mli {
		goto start
	}
	insertionI8(ar[lo:], no) // two insertion ranges
	return
}
...
func SortI8(ar []int64) {
	...
	start:
		n := lo + no
		l, pv, h := pivotI8(ar, lo, n, 3) // median-of-7
		l = partition1I8(ar, l, pv, h)
		n -= l
		no -= n + 1
		...
		if n < Mlr { // at least one not-long range?
			...
			shortI8(ar, lo, no) // we know no >= Mli
			return
		}
		...
		if atomic.AddUint32(&ngr, 1) == 0 { // increase goroutine counter
			panic("SortI8: long: counter overflow")
		}
	...
	for {
		...
		if n >= Mlr {
			if atomic.AddUint32(&ngr, 1) == 0 { // increase goroutine counter
				panic("SortI8: dual: counter overflow")
			}
			go glong(l, n)
