...
func insertionF8(ar []float64, hi int) {
...
// pivotF8 divides [lo..hi] range into 2n+1 equal intervals, sorts mid-points of them
...
func pivotF8(ar []float64, lo, hi, n int) (int, float64, int) {
	m := mid(lo, hi)
...
func partition1F8(ar []float64, l int, pv float64, h int) int {
	for {
...
func partition2F8(ar []float64, l, a int, pv float64, b, h int) (int, int) {
	for {
...
// returns m with ar[:m] <= pivot, ar[m:] >= pivot
func cdualparF8(par chan int, ar []float64, lo, hi int) int {

	lo, pv, hi := pivotF8(ar, lo, hi, 4) // median-of-9

	m := mid(lo, hi)
	a, b := mid(lo, m), mid(m, hi)

	go func(l, h int) {
		par <- partition1F8(ar, l, pv, h) // mid half range
	}(a, b)

	a, b = partition2F8(ar, lo, a-1, pv, b+1, hi) // left/right quarter ranges
	m = <-par
...
func shortF8(ar []float64, lo, no int) {
start:
	n := lo + no
	l, pv, h := pivotF8(ar, lo, n, 2) // median-of-5
	l = partition1F8(ar, l, pv, h)
	n -= l
	no -= n + 1
	...
	if n >= Mli {
		shortF8(ar, l, n) // recurse on the shorter range
		goto start
	}
	insertionF8(ar[l:], n) // at least one insertion range

	if no >= Mli {
		goto start
	}
	insertionF8(ar[lo:], no) // two insertion ranges
	return
}
...
func SortF8(ar []float64) {
	...
	start:
		n := lo + no
		l, pv, h := pivotF8(ar, lo, n, 3) // median-of-7
		l = partition1F8(ar, l, pv, h)
		n -= l
		no -= n + 1
		...
		if n < Mlr { // at least one not-long range?
			...
			shortF8(ar, lo, no) // we know no >= Mli
			return
		}
