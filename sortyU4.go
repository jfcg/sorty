...
func insertionU4(ar []uint32, hi int) {
...
// pivotU4 divides [lo..hi] range into 2n+1 equal intervals, sorts mid-points of them
...
func pivotU4(ar []uint32, lo, hi, n int) (int, uint32, int) {
	m := mid(lo, hi)
...
func partition1U4(ar []uint32, l int, pv uint32, h int) int {
	for {
...
func partition2U4(ar []uint32, l, a int, pv uint32, b, h int) (int, int) {
	for {
...
// returns m with ar[:m] <= pivot, ar[m:] >= pivot
func cdualparU4(par chan int, ar []uint32, lo, hi int) int {

	lo, pv, hi := pivotU4(ar, lo, hi, 4) // median-of-9

	m := mid(lo, hi)
	a, b := mid(lo, m), mid(m, hi)

	go func(l, h int) {
		par <- partition1U4(ar, l, pv, h) // mid half range
	}(a, b)

	a, b = partition2U4(ar, lo, a-1, pv, b+1, hi) // left/right quarter ranges
	m = <-par
...
func shortU4(ar []uint32, lo, no int) {
start:
	n := lo + no
	l, pv, h := pivotU4(ar, lo, n, 2) // median-of-5
	l = partition1U4(ar, l, pv, h)
	n -= l
	no -= n + 1
	...
	if n >= Mli {
		shortU4(ar, l, n) // recurse on the shorter range
		goto start
	}
	insertionU4(ar[l:], n) // at least one insertion range

	if no >= Mli {
		goto start
	}
	insertionU4(ar[lo:], no) // two insertion ranges
	return
}
...
func SortU4(ar []uint32) {
	...
	start:
		n := lo + no
		l, pv, h := pivotU4(ar, lo, n, 3) // median-of-7
		l = partition1U4(ar, l, pv, h)
		n -= l
		no -= n + 1
		...
		if n < Mlr { // at least one not-long range?
			...
			shortU4(ar, lo, no) // we know no >= Mli
			return
		}
